...
class PPOAgent:
    ...
    def update(self, num_value_epochs=100, num_policy_epochs=3, batch_size=64,
        ...
        #value_optimizer = optim.Adam(value_params, lr=self.value_lr)
        ...
        for value_epoch in tqdm(range(num_value_epochs)):
            ...
            for start in tqdm(range(0, num_samples, batch_size), leave = False):
                ...
                value_loss.backward()
                vgn = nn.utils.clip_grad_norm_(value_params, self.max_grad_norm)
        ...
        #policy_optimizer = optim.Adam(policy_params, lr=self.policy_lr)
        ...
        for _ in tqdm(range(num_policy_epochs)):
            ...
            for start in tqdm(range(0, num_samples, batch_size), leave = False):
                ...
                loss.backward()
                pgn = nn.utils.clip_grad_norm_(policy_params, self.max_grad_norm)
                policy_optimizer.step()

                total_policy_loss += policy_loss.item()
                total_entropy += entropy.mean().item()
                total_clip_fraction += ((ratio - 1.0).abs() > self.clip_epsilon).float().mean().item()
        ...
        return {
            ...
            "value_loss": total_value_loss / max(value_update_count, 1),
            ...
            "policy_grad_norm": total_policy_grad_norm / num_updates,
            "value_grad_norm": total_value_grad_norm / max(value_update_count, 1),
        }
