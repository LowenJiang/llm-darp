...
def train(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        epoch_actions = []
        epoch_step_costs = []  # track solver costs per step for diagnostics
        epoch_raw_rewards = []  # track raw rewards before clipping
        ...
        with tqdm(range(num_customers), desc=f"Epoch {epoch}/{num_epochs}", unit="step") as pbar:
            for step in pbar: # 30 steps in total; user_id is 1,2,...30
                ...
                # predicted mask
                if no_mask:
                    masks = torch.ones((num_envs, 16), dtype=torch.float32, device=device)
                else:
                ...
                # Diagnostics: track costs and raw rewards
                epoch_step_costs.append(info['current_cost'].clone())
                epoch_raw_rewards.append(adjusted_reward.clone())

                # Running reward normalization: scale rewards to ~unit variance
                ...
                reward_running_var += batch_var * batch_n + delta**2 * reward_count * batch_n / max(new_count, 1)
                ...
                epoch_improvement += adjusted_reward  # track raw for logging
        ...
        # Action histogram
        if epoch_actions:
            writer.add_histogram("actions/distribution", torch.tensor(epoch_actions), epoch)

        # Solver cost & reward diagnostics
        if epoch_step_costs:
...
def main():
    ...
    parser.add_argument("--no-mask", action="store_true", default=False,
                        help="Disable action masking (all 16 actions allowed)")
    parser.add_argument("--force-accept", action="store_true", default=False,
                        help="Force all proposed shifts to be accepted")
