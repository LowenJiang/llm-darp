...
def main():
    ...
    class PolicyTestEnv(PDPTWEnv):
        """Wrap PDPTWEnv to match the policy's expected step/reset signatures."""

        def reset(self, td=None, batch_size=None):
            td = super().reset(td=td, batch_size=batch_size)
            done = torch.zeros(td.batch_size, dtype=torch.bool, device=td.device)
            td.set("done", done)
            return td

        def step(self, td: TensorDict) -> TensorDict:
            next_td = super().step(td)
            return TensorDict({"next": next_td}, batch_size=next_td.batch_size)
    ...
    env = PolicyTestEnv(generator=generator)
    ...
    policy.eval()
    with torch.no_grad():
        result = policy(td, env, phase="test", decode_type="greedy", max_steps=500)
