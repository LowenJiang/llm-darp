...
class RolloutBaseline(REINFORCEBaseline):
    ...
    def wrap_dataset(self, dataset: Any, env: Any, batch_size: int = 64, device: Union[str, torch.device] = "cpu", **kwargs) -> Any:
        """Precompute baseline values and add to dataset as 'extra' field."""
        if self.policy is None:
            log.warning("Baseline policy not initialized. Skipping dataset wrapping.")
            return dataset

        log.info("Precomputing rollout baseline values for dataset...")
        rewards = self._rollout(self.policy, env, dataset, device=device).detach().cpu()

        # Add precomputed rewards to dataset
