...
df_decisions = pd.read_csv(_file_dir / "traveler_trip_types_res_7.csv")
...
class TravelerDataset(Dataset):
    def __init__(self, df_decisions = None):
        ...
        missing_cols = [col for col in flexibility_personalities if col not in df_decisions.columns]
        if missing_cols:
            raise ValueError(
                "TravelerDataset expects decision columns for each flexibility type. "
                f"Missing columns: {missing_cols}"
            )
        df_decisions["decision"] = df_decisions.apply(lambda row: row.loc[row["flexibility"]], axis = 1)
...
class OnlineTravelerDataset(Dataset):
    ...
    def _would_accept_action_csv(self, sample_row, flex_type_idx, action_idx,
        ...
        if decisions is None:
            print(key)
            return False #self._would_accept_action_hardcoded(action_idx, flex_type_idx, action_space_map)
...
def update_embedding_model(embedding_model, online_data, flexibility_personalities, action_space_map,
    ...
    #df_online_unique = df_online.drop_duplicates()
    #print("Training data size for embedding:", df_online_unique.shape[0], df_online.shape[0])
