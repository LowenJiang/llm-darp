...
def _get_expert_actions(batch, key: str):
    actions = None
    if hasattr(batch, "get"):
        actions = batch.get(key, None)
        if actions is None and key != "actions":
            actions = batch.get("actions", None)
    elif hasattr(batch, key):
        actions = getattr(batch, key)
    if actions is None:
        raise KeyError(f"Missing expert actions in batch (expected key '{key}').")
...
def train_epoch(
    ...
    bc_actions_key: str = "expert_actions",
):
    ...
    for batch_idx in tqdm(range(num_batches), desc="Train batches", unit="batch", leave=False):
        ...
        # Forward pass
        if behavior_cloning:
            action_device = state.device or state["current_node"].device
            expert_actions = _get_expert_actions(batch, bc_actions_key).to(action_device)
            if expert_actions.dim() == 1:
                expert_actions = expert_actions.unsqueeze(0)
            if expert_actions.size(0) != state.batch_size[0]:
                raise ValueError("Expert actions batch size does not match state batch size.")
            bc_max_steps = expert_actions.size(1)
            outputs = policy(
                state,
                env,
                phase="train",
                actions=expert_actions,
                max_steps=bc_max_steps,
            )
            if outputs.get("truncated"):
                raise ValueError("Expert action sequence ended before the episode finished.")
            outputs = trainer.calculate_loss(_clone_state(state), batch, outputs, behavior_cloning=True)
        else:
            outputs = policy(
                state,
                env,
...
def train(args: argparse.Namespace) -> None:
    ...
    for epoch in range(args.epochs):
        ...
        train_metrics = train_epoch(
            ...
            behavior_cloning=args.behavior_cloning,
            bc_actions_key=args.bc_actions_key,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--behavior-cloning",
        action="store_true",
        help="Use expert actions for supervised behavior cloning instead of REINFORCE.",
    )
    parser.add_argument(
        "--bc-actions-key",
        type=str,
        default="expert_actions",
        help="Dataset key containing expert action sequences.",
    )
