...
def train(args: argparse.Namespace) -> None:
    ...
    for step in range(args.steps):
        ...
        if args.baseline_update_every > 0 and (step + 1) % args.baseline_update_every == 0:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--log-every", type=int, default=1, help="Log interval.")
    parser.add_argument(
        "--baseline-update-every",
        type=int,
        default=5,
        help="Update rollout baseline every N steps (0 to disable).",
    )
