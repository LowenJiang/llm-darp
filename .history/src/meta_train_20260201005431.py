...
from ppo_loop.dvrp_env_1 import DVRPEnv
from ppo_agent_v2 import PPOAgent
from embedding import EmbeddingFFN, TravelerDataset, likelihood_loss
...
def get_masks(
    ...
    This mask is looked up from the augmented csv. 
...
def train(
    ...
    tb_log_dir = save_path / "tensorboard"
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with tqdm(range(num_customers), desc=f"Epoch {epoch}/{num_epochs}", unit="step") as pbar:
            for step in pbar:
                ...
                for b in range(num_envs):
                    online_data.append({
                        'traveler_id': user_ids_np[b],
                        'flexibility': target_flexs[b].item(),
                        'decision': int(accepted_np[b])
                    })
                print(accepted_np)
                obs = next_obs
        ...
        if len(online_data) > 100 and epoch % embedding_update_epoch == 0:
            ...
            dataset_train = TravelerDataset(df_decisions=df_train)
            dataset_eval = TravelerDataset(df_decisions=df_eval)
