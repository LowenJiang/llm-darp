...
def train(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch < 5:
            epsilon = 1
        decay = math.log(0.8 / 0.01)
        action_epsilon = 0.8 * math.exp(-decay * (epoch - 1) / max(num_epochs - 1, 1))
        action_epsilon = max(0.01, action_epsilon)
...
def test_online_dataset(
    ...
    for step in range(num_customers):
        decay = math.log(0.8 / 0.01) # action masking decays to 0.01
        action_epsilon = 0.8 * math.exp(-decay * step / max(num_customers - 1, 1))
        action_epsilon = max(0.01, action_epsilon)
        user_ids_np = np.full(num_envs, step + 1)
        with torch.no_grad():
