...
def train(args: argparse.Namespace) -> None:
    ...
    for step in range(args.steps):
        ...
        if outputs.get("truncated"):
            done_mask = outputs.get("done")
            if done_mask is None:
                log.warning("Truncated rollout without done mask; skipping corruption filtering.")
            else:
                done_mask = done_mask.detach().bool()
                if not done_mask.all():
                    dropped = (~done_mask).sum().item()
                    log.warning("Dropping %d unfinished episodes due to max_steps truncation.", dropped)
                    if not done_mask.any():
                        log.warning("All episodes truncated; skipping optimizer step.")
                        continue
                    # Filter corrupt episodes out of the training data.
                    if hasattr(batch, "__getitem__"):
                        batch = batch[done_mask]
                    if hasattr(state, "__getitem__"):
                        state = state[done_mask]
                    filtered_outputs = {}
                    for key, value in outputs.items():
                        if torch.is_tensor(value) and value.dim() > 0 and value.shape[0] == done_mask.shape[0]:
                            filtered_outputs[key] = value[done_mask]
                        else:
