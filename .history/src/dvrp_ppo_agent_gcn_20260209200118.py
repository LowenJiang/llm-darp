...
class TripRequestEmbeddingModel(nn.Module):
    ...
    def forward(self, states):
        ...
        # ---------------------------------------------
        # (4.1) zero-mask customers with all-zero input
        # ---------------------------------------------
...
class PPOAgent:
    ...
    def __init__(
        ...
        self.raw_rewards: List[float] = []  # Store raw rewards for statistics
    ...
    def select_action_batch(
            ...
            # 1. Convert State to Tensor (avoid CPU roundtrips if already a tensor)
    ...
    def store_reward(self, reward: float, done: bool) -> None:
        ...
        Optionally normalizes reward using running statistics.

        Args:
            reward: Reward received
            done: Whether episode terminated
        """
        # Store raw reward for statistics
        self.raw_rewards.append(reward)

        # Update running statistics and normalize if enabled
        if self.normalize_rewards:
            self.reward_rms.update(np.array([reward]))
            normalized_reward = self.reward_rms.normalize(np.array([reward]))[0]
            self.rewards.append(float(normalized_reward))
        else:
    ...
    def store_rewards_batch(self, rewards: np.ndarray, dones: np.ndarray) -> None:
        ...
        Optionally normalizes rewards using running statistics.

        Args:
            rewards: (batch_size,) array of rewards
            dones: (batch_size,) array of done flags
        """
        # Store raw rewards for statistics
        ...
        if self.normalize_rewards:
            ...
            for reward, done in zip(normalized_rewards, dones):
    ...
    def update(self, num_value_epochs=100, num_policy_epochs=3, batch_size=64,
        ...
        raw_rewards = torch.FloatTensor(self.raw_rewards).to(self.device)
        ...
        # ========================================
        # Compute reward statistics for logging
        # ========================================
        raw_reward_mean = raw_rewards.mean().item()
        raw_reward_std = raw_rewards.std().item()
        normalized_reward_mean = rewards.mean().item()
        normalized_reward_std = rewards.std().item()
        ...
        value_optimizer = optim.Adam([
            {"params": self.policy.value_network.backbone.parameters(), "lr": self.value_lr["backbone"]},
            {"params": self.policy.value_network.network.parameters(), "lr": self.value_lr["network"]}
        ])
        total_value_loss = 0.0
        value_update_count = 0
        old_values = None

        # Track returns statistics
        all_returns_mean = 0.0
        all_returns_std = 0.0
        returns_samples = 0
        ...
        for value_epoch in tqdm(range(num_value_epochs)):
            ...
            for start in tqdm(range(0, num_samples, batch_size), leave = False):
                ...
                # Track returns statistics (only on first epoch to avoid double counting)
                if value_epoch == 0:
                    all_returns_mean += batch_returns.mean().item() * len(batch_returns)
                    all_returns_std += batch_returns.std().item() * len(batch_returns)
                    returns_samples += len(batch_returns)
        ...
        # Compute average returns statistics
        if returns_samples > 0:
            all_returns_mean /= returns_samples
            all_returns_std /= returns_samples
        ...
        # Store pre-normalization advantage statistics
        ...
        # Store post-normalization advantage statistics
        adv_mean_after = advantages.mean().item()
        adv_std_after = advantages.std().item()
        ...
        policy_optimizer = optim.Adam([
            {"params": self.policy.policy_network.backbone.parameters(), "lr": self.policy_lr["backbone"]},
            {"params": self.policy.policy_network.network.parameters(), "lr": self.policy_lr["network"]}
        ])
        ...
        for _ in tqdm(range(num_policy_epochs)):
            ...
            for start in tqdm(range(0, num_samples, batch_size), leave = False):
                ...
                clipped = ((ratio < 1 - self.clip_epsilon) | (ratio > 1 + self.clip_epsilon)).float().mean()
                total_clipped_fraction += clipped.item()
        ...
        return {
            ...
            "value_loss": total_value_loss / value_update_count,
            ...
            # Returns statistics
            "returns_mean": all_returns_mean,
            "returns_std": all_returns_std,
            # Advantage statistics
            "advantage_mean_before_norm": adv_mean_before,
            "advantage_std_before_norm": adv_std_before,
            "advantage_mean_after_norm": adv_mean_after,
            "advantage_std_after_norm": adv_std_after,
            # PPO ratio statistics
            "ratio_mean": total_ratio_mean / num_updates,
            "ratio_std": total_ratio_std / num_updates,
            "clipped_fraction": total_clipped_fraction / num_updates,
        }
        ...
        torch.save(
            {
                ...
                # Save reward normalization statistics
    ...
    def load(self, path: str) -> None:
        ...
        # Restore reward normalization statistics if available
...
def test_agent():
    ...
    # Columns: [origin_idx, pickup_tw_early, pickup_tw_late, dest_idx, drop_tw_early, drop_tw_late]
    num_trips = 6
    state = np.zeros((num_trips, 6), dtype=np.float32)
    state[:, 0] = np.random.randint(0, num_nodes, size=num_trips)  # origin
    state[:, 1] = np.random.randint(0, 50, size=num_trips)  # pickup early
    state[:, 2] = np.random.randint(0, 50, size=num_trips)  # pickup late
    state[:, 3] = np.random.randint(0, num_nodes, size=num_trips)  # dest
