...
class PDPTWEnv(nn.Module):
    ...
    def _compute_action_mask(self, td: TensorDict) -> torch.Tensor:
        ...
        pending_dropoffs = torch.zeros_like(td["completed"])
        pending_dropoffs.scatter_(1, pending_schedule, pending_mask)
        can_dropoff = is_dropoff & pending_dropoffs
        
        base_mask = is_depot | can_pickup | can_dropoff
        ...
        mask = current_mask & lookahead_valid
        
        # Fallback safety: ensure depot is always available
        no_feasible = ~mask[..., 1:].any(dim=-1, keepdim=True)
        mask[..., 0] = mask[..., 0] | no_feasible.squeeze(-1)
...
if __name__ == "__main__":
    ...
    config = PDPTWConfig(
        num_customers=generator.num_customers,
        vehicle_capacity=generator.vehicle_capacity,
    )
    env = PDPTWEnv(config=config, generator=generator)
    ...
    print(f"Episode reward: {reward.tolist()}")
