...
def train(
    ...
    writer.add_text(
        ...
        (
            ...
            f"disable_action_masking={disable_action_masking}, force_accept={force_accept}"
        ),
    )
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        epoch_env_reward = torch.zeros(num_envs, device=device)
        epoch_baseline_reward = torch.zeros(num_envs, device=device)
        epoch_patience_penalty = torch.zeros(num_envs, device=device)
        epoch_solver_failed = torch.zeros(num_envs, device=device)
        epoch_mask_density_sum = 0.0
        epoch_mask_density_count = 0
        epoch_actions = []
        ...
        with tqdm(range(num_customers), desc=f"Epoch {epoch}/{num_epochs}", unit="step") as pbar:
            for step in pbar: # 30 steps in total; user_id is 1,2,...30
                ...
                if disable_action_masking:
                    ...
                    epoch_mask_density_sum += masks.mean().item()
                    epoch_mask_density_count += 1
                ...
                epoch_env_reward += env_reward
                epoch_baseline_reward += baseline_reward
                epoch_patience_penalty += info["patience_penalty"]
                epoch_solver_failed += info["solver_failed"].float()
        ...
        avg_env_reward = (epoch_env_reward / num_customers).mean().item()
        avg_baseline_reward = (epoch_baseline_reward / num_customers).mean().item()
        avg_patience_penalty = (epoch_patience_penalty / num_customers).mean().item()
        avg_solver_fail_rate = (epoch_solver_failed / num_customers).mean().item()
        ...
        writer.add_scalar("metrics/avg_env_reward", avg_env_reward, epoch)
        writer.add_scalar("metrics/avg_baseline_reward", avg_baseline_reward, epoch)
        ...
        writer.add_scalar("metrics/avg_patience_penalty", avg_patience_penalty, epoch)
        writer.add_scalar("metrics/avg_solver_fail_rate", avg_solver_fail_rate, epoch)
        writer.add_scalar("train/epsilon", epsilon, epoch)
        writer.add_scalar("train/action_epsilon", action_epsilon, epoch)
        if epoch_mask_density_count > 0:
            writer.add_scalar("metrics/avg_mask_density", epoch_mask_density_sum / epoch_mask_density_count, epoch)
        if epoch_actions:
            actions_np = np.concatenate(epoch_actions, axis=0)
            writer.add_histogram("actions", actions_np, epoch)
