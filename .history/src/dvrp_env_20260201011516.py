...
class DVRPEnv(gym.Env):
    ...
    SLICE_FIELDS = {"h3_indices", "time_windows", "demand", "locs", "action_mask", "visited", "user_id"}
    ...
    def step(
        ...
        for b in range(self.batch_size):
            ...
            if trip_metadata_list is not None:
                if isinstance(trip_metadata_list, list):
                    md = trip_metadata_list[b] if b < len(trip_metadata_list) else {}
                else:
...
def test_env():
    ...
    env = DVRPEnv(num_customers=5, batch_size=2, seed=42, traveler_decisions_path=decisions_path)
