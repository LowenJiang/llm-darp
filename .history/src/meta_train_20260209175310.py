...
def get_masks(
    ...
    # Temporarily allow all actions regardless of computed constraints.
    # Keep shape identical to computed masks.
    return torch.ones_like(masks)  # [batch_size, 16]
...
def train(
    ...
    agent = PPOAgent(
        ...
        action_dim=16,
        hidden_dim=512,
        policy_lr={"backbone": 3e-5, "network": 3e-4},  # Reduced learning rates
        value_lr={"backbone": 1e-4, "network": 1e-3},   # Higher LR for value network
        device=device,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with tqdm(range(num_customers), desc=f"Epoch {epoch}/{num_epochs}", unit="step") as pbar:
            for step in pbar: # 30 steps in total; user_id is 1,2,...30
                ...
                action_np = agent.select_action_batch(current_state, masks=masks)
        ...
        stats = agent.update(
            num_value_epochs=20,  # Increased from 10 for better value function fitting
            num_policy_epochs=6,
            batch_size=64,
            num_envs=num_envs,
        ...
        if stats:
            ...
            # Reward statistics
            writer.add_scalar("rewards/raw_mean", stats.get("raw_reward_mean", 0.0), epoch)
            writer.add_scalar("rewards/raw_std", stats.get("raw_reward_std", 0.0), epoch)
            writer.add_scalar("rewards/normalized_mean", stats.get("normalized_reward_mean", 0.0), epoch)
            writer.add_scalar("rewards/normalized_std", stats.get("normalized_reward_std", 0.0), epoch)

            # Returns statistics
            writer.add_scalar("returns/mean", stats.get("returns_mean", 0.0), epoch)
            writer.add_scalar("returns/std", stats.get("returns_std", 0.0), epoch)

            # Advantage statistics (before and after normalization)
            writer.add_scalar("advantages/mean_before_norm", stats.get("advantage_mean_before_norm", 0.0), epoch)
            writer.add_scalar("advantages/std_before_norm", stats.get("advantage_std_before_norm", 0.0), epoch)
            writer.add_scalar("advantages/mean_after_norm", stats.get("advantage_mean_after_norm", 0.0), epoch)
            writer.add_scalar("advantages/std_after_norm", stats.get("advantage_std_after_norm", 0.0), epoch)

            # PPO ratio statistics
            writer.add_scalar("ppo/ratio_mean", stats.get("ratio_mean", 0.0), epoch)
            writer.add_scalar("ppo/ratio_std", stats.get("ratio_std", 0.0), epoch)
            writer.add_scalar("ppo/clipped_fraction", stats.get("clipped_fraction", 0.0), epoch)
        ...
        if epoch % log_interval == 0:
            ...
            print(f"[Epoch {epoch}/{num_epochs}] Rew: {avg_reward:.2f} | AgentCost: {avg_cost_agent:.1f} | Acc: {avg_acc:.2%}")
...
def test_online_dataset(
    ...
    agent = PPOAgent(
        ...
        action_dim=16,
        hidden_dim=512,
        policy_lr={"backbone": 3e-5, "network": 3e-4},
