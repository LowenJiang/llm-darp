...
class DVRPEnv(gym.Env):
    ...
    def _apply_perturbation_batch(
        ...
        # Ensure late >= early + 1 to avoid zero-width time windows
        # that cause NaN in the neural solver's attention mechanism
        tw[:, 0, 1] = torch.max(tw[:, 0, 1], tw[:, 0, 0] + 1)  # pickup late >= pickup early + 1
        tw[:, 1, 1] = torch.max(tw[:, 1, 1], tw[:, 1, 0] + 1)  # dropoff late >= dropoff early + 1
