...
if __name__ == "__main__":
    ...
    # Test Data Creation with H3 indices and travel time matrix
    # Mock travel time matrix: 7 nodes mapped to first 7 H3 indices
    num_test_nodes = 7
    mock_matrix_size = 10  # Small mock matrix
    mock_travel_time_matrix = torch.zeros(mock_matrix_size, mock_matrix_size, dtype=torch.float32)
    
    # Fill with simple distances (in minutes)
    for i in range(mock_matrix_size):
        for j in range(mock_matrix_size):
            if i != j:
                mock_travel_time_matrix[i, j] = abs(i - j) * 10.0  # 10 min per index difference
    
    td_test = TensorDict(
        {
            # H3 indices for: Depot, P1, D1, P2, D2, P3, D3
            "h3_indices": torch.tensor([[0, 1, 2, 3, 4, 5, 6]], dtype=torch.int64),
            "travel_time_matrix": mock_travel_time_matrix.unsqueeze(0),  # [1, 10, 10]
            ...
            # Demand: 0 for depot, +load for pickup, -load for delivery
            "demand": torch.tensor([[0, 1, -1, 1, -1, 1, -1]], dtype=torch.float32),
            "vehicle_capacity": torch.tensor([[4]], dtype=torch.int64),
        },
        batch_size=torch.Size([1]),
    )

    print("Running DARP Solver with H3 indices...")
    result = darp_solver(td_test, max_vehicles=2, time_limit_seconds=5)
    print(f"Status: Found {result['vehicles_used']} routes.")
    print(f"Total Travel Time: {result['total_time']:.3f} minutes")
