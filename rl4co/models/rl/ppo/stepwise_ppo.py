...
# Try to import torchrl replay buffers (optional dependency for StepwisePPO)
try:
    ...
    log.warning(
        f"torchrl replay buffers not available: {e}. "
        "StepwisePPO will not be functional. "
        "Install compatible torchrl version or use regular PPO instead."
    )
...
def make_replay_buffer(buffer_size, batch_size, device="cpu"):
    if not TORCHRL_AVAILABLE:
        raise ImportError(
            "torchrl replay buffers not available. "
            "Please install a compatible version of torchrl or use regular PPO."
        )
    if device == "cpu":
