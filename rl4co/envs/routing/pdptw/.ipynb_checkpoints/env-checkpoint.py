...
from rl4co.utils.ops import gather_by_index, get_geo_distance, get_geod_tour_length
...
class PDPTWEnv(CVRPTWEnv):
    ...
    def _make_spec(self, generator):
        ...
        if not hasattr(generator, 'num_customers'):
            # Parent is calling with wrong generator type during init chain
            # Just return early - we'll rebuild specs after with correct generator
            return
    ...
    def _reset(self, td: Optional[TensorDict] = None, batch_size: Optional[list] = None) -> TensorDict:
        ...
        # SFGenerator outputs:
        #   - locs [B, 2*num_customers, 2] (NO depot)
        #   - time_windows [B, 2*num_customers+1, 2] (depot already included!)
        #   - demand [B, 2*num_customers] (NO depot)
        # We need to check each tensor independently

        # Add depot to locs if not present
        if td["locs"].shape[-2] == self.generator.num_customers * 2:
            locs = torch.cat((td["depot"][..., None, :], td["locs"]), -2)
        else:
            locs = td["locs"]

        # Add depot to demands if not present
        if td["demand"].shape[-1] == self.generator.num_customers * 2:
            demands = torch.cat((torch.zeros(*batch_size, 1, device=device, dtype=td["demand"].dtype), td["demand"]), -1)
        else:
            demands = td["demand"]

        # Check if time_windows already has depot (should be 2*num_customers+1)
        if td["time_windows"].shape[-2] == self.generator.num_customers * 2 + 1:
            # Depot already included
            tws = td["time_windows"]
        elif td["time_windows"].shape[-2] == self.generator.num_customers * 2:
            # Depot not included, add it
            depot_tw = torch.stack([
                torch.zeros(*batch_size, 1, device=device, dtype=td["time_windows"].dtype),
                torch.full((*batch_size, 1), 1440.0, device=device, dtype=td["time_windows"].dtype)
            ], dim=-1)  # [B, 1, 2]
            tws = torch.cat((depot_tw, td["time_windows"]), -2)
        else:
            tws = td["time_windows"]

        # Safety check: ensure dimensions match
        num_locs = locs.shape[-2]
        if tws.shape[-2] != num_locs:
            raise ValueError(
                f"Dimension mismatch: locs has {num_locs} locations but time_windows has {tws.shape[-2]}. "
                f"Generator output shapes - locs: {td['locs'].shape}, time_windows: {td['time_windows'].shape}, "
                f"num_customers: {self.generator.num_customers}"
            )
        if demands.shape[-1] != num_locs:
            raise ValueError(
                f"Dimension mismatch: locs has {num_locs} locations but demands has {demands.shape[-1]}. "
                f"Generator output shapes - locs: {td['locs'].shape}, demand: {td['demand'].shape}"
            )
    ...
    def get_action_mask(self, td: TensorDict) -> torch.Tensor:
        ...
        # Only apply for actual dropoffs (not depot)
        ...
        # Note: demands shape is [B, N] and remaining_cap is [B, 1]
        # For dropoffs, demand is negative, so always feasible capacity-wise
        # For pickups, demand is positive and must fit in remaining capacity
        # Depot has 0 demand, so always feasible
        capacity_mask = (demands <= remaining_cap) | is_dropoff | is_depot
