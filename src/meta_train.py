...
def train(
    ...
    traveler_decisions_path = str(decisions_path)
    model_path = solver_ckpt

    num_epochs = max(1, num_episodes // num_envs) # epochs
    print("="*50)
    ...
    # baseline_env = copy.deepcopy(env) # No need for baseline env! 

    # 2. Initialize Agent
    tt_matrix = env.data_generator.generator.travel_time_matrix.to(device) # 31*31
    traveler_decisions = pd.read_csv(traveler_decisions_path)
    
    #tt_matrix = 1/(tt_matrix+ torch.eye(tt_matrix.shape[0]))
    sigma = tt_matrix[tt_matrix>0].median()
    tt_matrix = torch.exp(-tt_matrix**2 / sigma**2)
    #tt_matrix = tt_a.fill_diagonal_(0)

    print(tt_matrix)
    #print(f"{1/tt_matrix+ np.eye(tt_matrix[0])*2 }")
    agent = PPOAgent(
        travel_time_matrix=tt_matrix,
        gcn_hidden=128,
        gcn_out=32,
        time_embed_dim=16,
        time_vocab_size=22, #! 1500-> 20 , matching the 20 time slots for state space
    ...
    online_data = deque(maxlen=300000) # 500*30*20 = 300000
    ...
    total_steps = 0
    embedding_update_epoch = 20

    print("Starting training loop...")

    # --- Training Loop ---
    for epoch in range(1, num_epochs + 1):
        # Calculate the pre-perturbation cost
        ...
        #epoch_accepted = torch.zeros(num_envs, device=device) # Each epoch run 128 parallel envs

        rl_reward = []
        acceptance = []
        epsilon = 0 #initial_epsilon - (initial_epsilon - final_epsilon) * (epoch - 1) / max(num_epochs - 1, 1)
        if epoch <= embedding_update_epoch:
            epsilon = 0
            action_epsilon = 1 - epoch/embedding_update_epoch # mask has no weights
        else:
            action_epsilon = 0.0
        ...
        with tqdm(range(num_customers), desc=f"Epoch {epoch}/{num_epochs}", unit="step") as pbar:
            for step in pbar: # 30 steps in total; user_id is 1,2,...30
                ...
                with torch.no_grad():
                    ...
                    pred_proba = embedding_model(u_embed_ids) # start from 0? 

                    dist = torch.distributions.Categorical(probs=pred_proba)
                    target_flexs = dist.sample() 
                    ...
                    #target_flexs = torch.where(explore_mask, random_flexs, predicted_flexs) #Now its all target flexes

                # predicted mask 
                #! This line need to check

                masks = get_masks(env, user_ids_np, target_flexs, decisions=traveler_decisions, device='cpu') # [batch_size, 16(actions)]
                # returning (num_cus, num_env)

                #current_state = prepare_agent_observation(obs)
                #action_np = agent.select_action_batch(current_state, masks=masks, epsilon=action_epsilon) #action_epsilon: applied on the logits
                #print(obs)

                if action_epsilon > 0:
                    action = agent.select_action_batch(obs, masks=None, epsilon=action_epsilon)
                else:
                    action = agent.select_action_batch(obs, masks=masks, epsilon=action_epsilon)

                # print mask
                #action_tensor = torch.tensor(action_np, device=device, dtype=torch.long)
                #next_obs, env_reward, term, trunc, info = env.step(action_tensor) #! Here the reward is taken as step diff, telescopable
                obs, reward, accepted, done, info = env.step(action)

                rl_reward.append(reward)
                acceptance.append(accepted)

                
                #epoch_accepted += info['accepted'].float() # this needs to be checked : The acceptance rate is not normal. What is the accepted format?
                #print accepted
                ...
                #! adjusted reward must be changed
        ...
        # Write: acceptance rate, improvement rate, average baseline cost, average perturbed cost
        final_cost = np.array(rl_reward).sum(axis=0)
        imp_all = (final_cost + baseline_cost)/baseline_cost 
        imp = np.mean((final_cost + baseline_cost)/baseline_cost )
        acc = np.mean(np.array(acceptance).sum(axis=0)/num_customers)
        print(f"{imp=}")
        print(f"{acc=}")
        writer.add_scalar("stats/improvement", imp*100, epoch)
        writer.add_scalar("stats/acceptance", acc*100, epoch)
        writer.add_scalar("stats/largest_imp", np.max(imp_all)*100, epoch)
        writer.add_scalar("stats/smallest_imp", np.min(imp_all)*100, epoch)
        #print(f"{final_cost=}")
        ...
        if len(online_data) > 0 and epoch % 10 == 0:
            ...
            if len(df_online) < 10:
                ...
                split_idx = int(0.9 * len(df_online))
...
def test_online_dataset(
    ...
    agent = PPOAgent(
        travel_time_matrix=1/tt_matrix,
        gcn_hidden=32,
        gcn_out=32,
        time_embed_dim=16,
...
def main():
    ...
    parser.add_argument("--ckpt-dir", type=str, default="./checkpoints/meta_train_test")
    parser.add_argument("--log-dir", type=str, default="./runs/meta_train_test")
    parser.add_argument("--resume", type=str, default=None)
    parser.add_argument("--solver-ckpt", type=str, default="./checkpoints/refined/best.pt")
