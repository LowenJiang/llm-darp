...
# Minimum service time (minutes) at each customer node.  Prevents subtours
# in the Big-M time-linking formulation when travel time between a
# pickup-delivery pair is zero (same H3 cell).
...
def _solve_routing(td, max_vehicles: int, time_limit: int = 5):
    ...
    for i in N:
        for j in N:
            ...
            if e[i] + c[i, j] + SERVICE_TIME > ld[j]:  # time-infeasible arc
                continue
            arcs.append((i, j))

    M = {(i, j): max(0.0, ld[i] + c[i, j] + SERVICE_TIME - e[j])
         for (i, j) in arcs}

    # adjacency lists for fast constraint iteration
    ...
    for i in P:
        mdl.addConstr(
            gp.quicksum(x[i, j, k]
                        for j in out_nbrs[i] for k in K) == 1)
    ...
    for i in P + D:
        for k in K:
            mdl.addConstr(B[i, k] >= e[i])
            mdl.addConstr(B[i, k] <= ld[i])
    for k in K:
    ...
    # travel-time linking (per-arc Big-M) with SERVICE_TIME to prevent subtours
    for (i, j) in arcs:
        for k in K:
            mdl.addConstr(
                B[j, k] >= B[i, k] + c[i, j] + SERVICE_TIME
                - M[i, j] * (1 - x[i, j, k]))

    # precedence: delivery after pickup + service + direct travel
    for r in P:
        d_node = n + r
        t_pd = c[r, d_node]
        M_prec = max(0.0, ld[r] + t_pd + SERVICE_TIME - e[d_node])
        for k in K:
            mdl.addConstr(
                B[d_node, k] >= B[r, k] + t_pd + SERVICE_TIME
...
def solve_joint_mip(
    ...
    Returns dict with keys: total_cost, selected_actions, routes.
    """
    batch_idx = 0
    ...
    for i in N:
        for j in N:
            if i == j:              continue
            if i == dep_end:        continue
            if j == 0:              continue
            if e_wide[i] + c[i, j] + SERVICE_TIME > l_wide[j]:
                continue
            arcs.append((i, j))

    M_arc = {(i, j): max(0.0, l_wide[i] + c[i, j] + SERVICE_TIME - e_wide[j])
             for (i, j) in arcs}
    ...
    # depot time windows (no action dependence)
    for k in K:
    ...
    for r in P:
        mdl.addConstr(
            gp.quicksum(y[r, li] for li in range(len(action_tw[r]))) == 1)
    ...
    for r in P:
        d_node = n + r
        t_pd = c[r, d_node]
        M_prec = max(0.0, l_wide[r] + t_pd + SERVICE_TIME - e_wide[d_node])
        for k in K:
            mdl.addConstr(
                B[d_node, k] >= B[r, k] + t_pd + SERVICE_TIME
                - M_prec * (1 - gp.quicksum(
...
def solve_optimal(
    ...
    if fix_action is not None:
        ...
        cost, routes = _solve_routing(td_mod, max_vehicles,
    ...
    best_cost, best_routes = _solve_routing(td_cur, max_vehicles,
    ...
    for it in range(1, max_iterations + 1):
        ...
        for r in range(1, n_customers + 1):
            ...
            for a_idx, ps, ds in accepted_actions.get(r, [(12, 0, 0)]):
                ...
                cost, routes = _solve_routing(td_trial, max_vehicles,
                                              time_limit=time_limit_per_solve)
...
def main() -> None:
    ...
    print("=" * 60)
    print(f"{'Req':>4} {'Trav':>5} {'Act':>4} {'PShift':>7} {'DShift':>7}  "
    ...
    for r in range(1, n_cust + 1):
        ...
        print(
            f"{r:4d} {trav_id:5d} {a_idx:4d} {ps:+5d}m  {ds:+5d}m   "
