...
DEFAULT_FREE_VEHICLES = 5
DEFAULT_EXTRA_VEHICLE_PENALTY = 200.0


def count_vehicles_from_actions(actions: torch.Tensor) -> torch.Tensor:
    """Count contiguous non-zero segments (vehicles) separated by depots (0)."""
    if actions.numel() == 0:
        return torch.zeros(actions.shape[:-1], dtype=torch.long, device=actions.device)
    nonzero = actions.ne(0)
    prev_nonzero = torch.zeros_like(nonzero)
    prev_nonzero[..., 1:] = nonzero[..., :-1]
    segment_starts = nonzero & ~prev_nonzero
    return segment_starts.sum(dim=-1)


def apply_vehicle_penalty(
    reward: torch.Tensor,
    actions: torch.Tensor,
    free_vehicles: int = DEFAULT_FREE_VEHICLES,
    penalty_per_extra: float = DEFAULT_EXTRA_VEHICLE_PENALTY,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    vehicles_used = count_vehicles_from_actions(actions)
    overage = (vehicles_used - free_vehicles).clamp(min=0)
    penalty = -overage.to(dtype=reward.dtype, device=reward.device) * float(penalty_per_extra)
    reward = reward + penalty
    feasible = vehicles_used <= free_vehicles
    return reward, vehicles_used, penalty, feasible


def apply_vehicle_penalty_to_outputs(
    ...
    if reward is None or actions is None:
        return outputs
    reward, vehicles_used, penalty, feasible = apply_vehicle_penalty(
        reward,
        actions,
    ...
    outputs.update(
        {
            ...
            "feasibility": feasible.to(dtype=reward.dtype),
        }
    )
    return outputs
...
class RolloutBaseline(REINFORCEBaseline):
    ...
    def epoch_callback(
        ...
        **kwargs,
        ...
        log.info(f"Candidate mean: {candidate_mean:.4f}, Baseline mean: {self.mean:.4f}")
        ...
        if ttest_rel is not None and self.bl_vals is not None:
            ...
            log.info(f"t-statistic: {t_stat:.4f}, p-value: {p_val_one_sided:.4f}")

            if t_stat <= 0:
                update_baseline = False
            else:
                update_baseline = p_val_one_sided < self.bl_alpha

        if update_baseline:
            log.info("✓ Updating baseline policy")
            self._update_policy(policy, env, batch_size=batch_size, device=device, dataset_size=dataset_size)
    ...
    def wrap_dataset(
        ...
        **kwargs,
        ...
        """Precompute baseline values in batches and add to dataset as 'extra'."""
        if self.policy is None:
            log.warning("Baseline policy not initialized. Skipping dataset wrapping.")
            return dataset

        num_instances = self._get_num_instances(dataset)
        if num_instances == 0:
            log.warning("Empty dataset provided for baseline precompute.")
            return dataset

        log.info("Precomputing rollout baseline values for %d instances...", num_instances)
        rewards = self._rollout_batched(self.policy, env, dataset, num_instances, batch_size, device=device)

        if hasattr(dataset, "set"):
            dataset.set("extra", rewards)
        else:
            dataset.extra = rewards

        return dataset

    @staticmethod
    def _get_num_instances(dataset: Any) -> int:
        if hasattr(dataset, "batch_size"):
            return int(dataset.batch_size[0])
    ...
    def _rollout_batched(
        ...
        for start in range(0, num_instances, batch_size):
            ...
            rewards = self._rollout(policy, env, batch, device=device).detach().cpu()
...
class WarmupBaseline(REINFORCEBaseline):
    """Warmup baseline: convex combination of rollout and exponential baseline.
    
    During the first n_epochs, gradually transitions from exponential baseline
    to the main baseline (e.g., rollout).
    
    Args:
        baseline: Main baseline to use after warmup
        n_epochs: Number of epochs to warmup (default: 1)
        warmup_exp_beta: Beta for exponential baseline during warmup (default: 0.8)
    """

    def __init__(self, baseline: REINFORCEBaseline, n_epochs: int = 1, warmup_exp_beta: float = 0.8):
        super().__init__()
        self.baseline = baseline
        self.warmup_baseline = ExponentialBaseline(beta=warmup_exp_beta)
        self.alpha = 0  # 0 = full warmup, 1 = full baseline
        self.n_epochs = n_epochs

    def setup(self, *args, **kwargs):
        """Setup the main baseline."""
        self.baseline.setup(*args, **kwargs)

    def wrap_dataset(self, dataset, *args, **kwargs):
        """Wrap dataset - only if alpha > 0 (past warmup phase)."""
        if self.alpha > 0:
            return self.baseline.wrap_dataset(dataset, *args, **kwargs)
        return dataset  # Warmup baseline doesn't precompute

    def eval(self, state, reward, env=None):
        """Evaluate baseline - convex combination during warmup."""
        if self.alpha == 1:
            return self.baseline.eval(state, reward, env)
        if self.alpha == 0:
            return self.warmup_baseline.eval(state, reward, env)

        # Convex combination during warmup
        v_b, l_b = self.baseline.eval(state, reward, env)
        v_wb, l_wb = self.warmup_baseline.eval(state, reward, env)
        return (
            self.alpha * v_b + (1 - self.alpha) * v_wb,
            self.alpha * l_b + (1 - self.alpha) * l_wb
        )

    def epoch_callback(self, *args, epoch=None, **kwargs):
        """Update alpha and call main baseline's epoch callback."""
        # Always update the main baseline
        self.baseline.epoch_callback(*args, epoch=epoch, **kwargs)

        # Update warmup alpha
        if epoch is not None and epoch < self.n_epochs:
            self.alpha = (epoch + 1) / self.n_epochs
            log.info(f"Warmup alpha = {self.alpha:.3f}")
        elif epoch is not None and epoch >= self.n_epochs:
            self.alpha = 1.0
...
def get_reinforce_baseline(name: Optional[str], **kwargs) -> REINFORCEBaseline:
    ...
    Special handling for 'rollout':
    - Creates WarmupBaseline wrapping RolloutBaseline (matching rl4co behavior)
    - Default: 1 epoch warmup with exponential baseline (beta=0.8)
    """
    if name is None:
        name = "no"

    # Special case: "rollout" creates WarmupBaseline(RolloutBaseline)
    if name == "rollout":
        warmup_epochs = kwargs.get("n_epochs", 1)
        warmup_exp_beta = kwargs.get("exp_beta", 0.8)
        bl_alpha = kwargs.get("bl_alpha", 0.05)

        rollout_baseline = RolloutBaseline(bl_alpha=bl_alpha)
        return WarmupBaseline(
            baseline=rollout_baseline,
            n_epochs=warmup_epochs,
            warmup_exp_beta=warmup_exp_beta
        )

    # Handle "warmup" with custom inner baseline
    if name == "warmup":
        inner_baseline = kwargs.pop("baseline", "rollout")
        if isinstance(inner_baseline, str):
            inner_baseline = get_reinforce_baseline(inner_baseline, **kwargs)
        return WarmupBaseline(inner_baseline, **kwargs)
...
class REINFORCE:
    ...
    def calculate_loss(
            ...
            policy_out.update(
                {
                    ...
                    "feasibility": feasibility.to(dtype=reward.dtype),
                }
            )

        # Check for precomputed baseline values in batch["extra"]
        extra = None
        if hasattr(batch, "get"):
            extra = batch.get("extra", None)
        elif hasattr(batch, "extra"):
            extra = batch.extra
        ...
        if extra is None:
            # No precomputed baseline - compute on the fly (slower)
            baseline_value, baseline_loss = self.baseline.eval(initial_state, reward, self.env)
        else:
            # Use precomputed baseline values (faster)
            if isinstance(extra, torch.Tensor):
                baseline_value = extra.to(reward.device)
            else:
                baseline_value = extra
            baseline_loss = torch.tensor(0.0, device=reward.device)
    ...
    def _collect_metrics(self, outputs: Dict[str, Any], phase: str, dataloader_idx: Optional[int]) -> Dict[str, Any]:
        ...
        if feasibility is not None:
            metrics[f"{phase}_feasibility_rate"] = feasibility.detach().float().mean()
...
__all__ = [
    ...
    "count_vehicles_from_actions",
    "apply_vehicle_penalty",
    "apply_vehicle_penalty_to_outputs",
]
