...
class ActorCritic(nn.Module):
    ...
    Input: State (30, 8) flattened to 240-dimensional vector
    ...
    def __init__(
        self,
        state_dim: int = 240,  # 30 requests * 8 features
        action_dim: int = 16,
        hidden_dim: int = 256,
    ):
        """
        Args:
            state_dim: Flattened state dimension (default: 240)
    ...
    def forward(self, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            state: State tensor of shape (batch_size, 30, 8)

        Returns:
            action_probs: Action probabilities (batch_size, 16)
            state_value: State value (batch_size, 1)
        """
        # Flatten state: (batch_size, 30, 8) -> (batch_size, 240)
    ...
    def mask_action(self, action_probs, mask, eps = 0):
        masked_action_probs = mask * action_probs
        masked_action_probs = masked_action_probs / masked_action_probs.sum()
        return eps * action_probs + (1 - eps) * masked_action_probs
    ...
    def act(self, state: torch.Tensor) -> Tuple[int, torch.Tensor, torch.Tensor]:
        ...
        Args:
            state: State tensor of shape (30, 8)
        ...
        ## TODO: Obtain masked action probs by calling self.mask_action(action_probs, mask, eps), where eps should start with 0.2 and gradually decreases to 0 as policy iteration increases
    ...
    def evaluate(
        ...
        Args:
            states: Batch of states (batch_size, 30, 8)
...
class PPOAgent:
    ...
    def select_action(self, state: np.ndarray) -> int:
        ...
        Args:
            state: State array of shape (30, 8)
...
def test_agent():
    ...
    agent = PPOAgent(state_dim=240, action_dim=16, hidden_dim=128)

    # Create dummy state
    state = np.random.randn(30, 8).astype(np.float32)
